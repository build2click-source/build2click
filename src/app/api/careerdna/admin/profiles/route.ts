...
/**
 * Handle Occupational Profile CRUD Operations for CareerDNA
 */
export async function GET() {
  try {
    ...
    const profiles = await careerdnaPrisma.occupationalProfile.findMany({
      orderBy: { title: "asc" }
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const profile = await careerdnaPrisma.occupationalProfile.upsert({
    ...
    console.error("POST /api/careerdna/admin/profiles error:", error);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await careerdnaPrisma.occupationalProfile.delete({ where: { id } });
