...
export default function CareerDNAAdminAttemptDetailsPage() {
  ...
  useEffect(() => {
    if (authStatus === "unauthenticated") { 
      router.push("/careerdna/login"); 
      return; 
    }
    if (authStatus !== "authenticated") return;
    if ((session?.user as any)?.role !== "ADMIN") { 
      router.push("/careerdna/dashboard"); 
      return; 
    }

    if (id) {
      fetch(`/api/careerdna/admin/attempts/${id}`)
        .then((r) => r.json())
        .then((data) => {
          setAttempt(data);
          setLoading(false);
  ...
  if (!attempt || attempt.error) {
    return (
      <div className="min-h-screen bg-[#F4F7F6] flex items-center justify-center flex-col gap-4">
        ...
        <Link href="/careerdna/admin" className="text-[#fb6a51] hover:underline">← Back to Admin</Link>
      </div>
    );
  }
  ...
  const groupedResponses = attempt.responses.reduce((acc: any, response: any) => {
    ...
    acc[modId].total += 1;
    if (response.isCorrect) acc[modId].correct += 1;
    if (response.scoreValue != null) acc[modId].scoreSum += response.scoreValue;
    return acc;
  }, {});
  ...
  return (
    <div className="bg-[#f8f6f5] text-[#2D3142] min-h-screen font-sans">
      <div className="max-w-[900px] mx-auto px-6 py-10 pb-20">
        <Link href="/careerdna/admin" className="text-sm font-bold text-[#fb6a51] hover:underline mb-6 inline-block">
              ...
              <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
                ...
                <ResponsiveContainer width="100%" height={280}>
                  <RadarChart data={resultsData.radarData}>
                    ...
                    <Radar name="Profile" dataKey="score" stroke="#fb6a51" fill="#fb6a51" fillOpacity={0.2} strokeWidth={2} />
                    <Tooltip formatter={(v) => [`${v}%`, "Score"]} contentStyle={{ borderRadius: "12px", border: "1px solid #f1f5f9", fontSize: "13px" }} />
                  </RadarChart>
                </ResponsiveContainer>
              </div>
                  ...
                  {Object.entries(resultsData.big5).map(([trait, score]) => (
                    <div key={trait} className="flex flex-col gap-1">
                      ...
                      <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-[#fb6a51] to-[#f97316] rounded-full transition-all duration-1000 ease-out" style={{ width: `${score}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <section className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
                  {resultsData.cognitiveBreakdown.map((c: any) => (
                    <div key={c.area} className="flex flex-col items-center gap-3 p-4 md:p-6 rounded-2xl bg-[#f8f6f5]">
                      <div className="relative size-16 md:size-20">
                        <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                          <circle cx="50" cy="50" r="40" fill="none" stroke="#f1f5f9" strokeWidth="10" />
                          <circle cx="50" cy="50" r="40" fill="none" stroke="#fb6a51" strokeWidth="10" strokeLinecap="round" strokeDasharray={`${2 * Math.PI * 40}`} strokeDashoffset={`${2 * Math.PI * 40 * (1 - c.pct / 100)}`} />
                        </svg>
                ...
                >
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-3 text-sm text-[#9095A7]">
                      <span className="px-2.5 py-1 rounded-full bg-slate-100 text-xs font-bold text-slate-600">{mod.type}</span>
                  ...
                  <div className="mt-4 md:mt-0 px-4 py-2 bg-slate-50 text-slate-500 rounded-full font-bold text-sm border border-slate-100">
                      ...
                      <tbody>
                        {mod.responses.map((response: any) => (
                          <tr key={response.id} className="border-b border-slate-50 hover:bg-[#f8f6f5] transition-colors text-[13px]">
                            <td className="px-6 py-4"><div className="font-medium text-slate-800 line-clamp-3">{response.question.text}</div></td>
                            <td className="px-6 py-4">
                              <div className="font-medium text-slate-600">{response.question.traitDimension || "N/A"}</div>
                              {response.question.scoringPolarity && <div className="text-[11px] font-mono text-slate-400 mt-1">{response.question.scoringPolarity}</div>}
                            </td>
                            <td className="px-6 py-4"><span className="font-bold text-[#fb6a51]">{response.answerText}</span></td>
