...
export default function CareerDNAAdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/careerdna/admin/stats")
      .then((r) => r.json())
                  ...
                  {stats.candidates.map((c) => (
                    <tr key={c.id} className="border-b border-[#f4e8e6] hover:bg-[#f8f6f5] transition-colors">
                      ...
                      <td className="px-4 py-5 text-right">
                        {c.attemptId ? (
                          <Link href={`/careerdna/admin/attempts/${c.attemptId}`} className="text-sm font-bold text-[#fb6a51] hover:underline">
                            View Details
                          </Link>
                        ) : (
