...
export default function CareerDNAAdminProfilesPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/careerdna/admin/profiles")
      .then((r) => r.json())
      .then((data) => { 
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/careerdna/admin/profiles?id=${id}`, { method: "DELETE" });
  ...
  return (
    <div className="bg-[#F4F7F6] text-[#2D3142] min-h-screen font-sans">
      <div className="w-full max-w-[1100px] mx-auto px-6 pb-20">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-10 pb-8">
          <Link
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <Link
            href="/careerdna/admin/profiles/import"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {profiles.map((p) => {
             let vectorSize = 0;
             try { vectorSize = JSON.parse(p.targetVector).length; } catch(e) {}
                ...
                <div>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex gap-1 shrink-0 bg-slate-50 p-1 rounded-xl">
                      <Link
                        href={`/careerdna/admin/profiles/${p.id}`}
