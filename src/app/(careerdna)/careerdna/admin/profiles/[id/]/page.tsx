...
export default function CareerDNAEditProfilePage() {
  ...
  useEffect(() => {
    ...
    if (profileId) {
      fetch(`/api/careerdna/admin/profiles/${profileId}`)
        .then(r => r.json())
        .then(data => {
          if (!data.error) {
            setProfile({
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/careerdna/admin/profiles", {
        method: "POST",
      ...
      if (res.ok) router.push("/careerdna/admin/profiles");
  ...
  if (!profile) {
    return (
      <div className="min-h-screen bg-[#F4F7F6] flex flex-col items-center justify-center gap-4">
        ...
        <Link href="/careerdna/admin/profiles" className="text-[#fb6a51] font-bold">← Back</Link>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[#F4F7F6] text-[#2D3142] min-h-screen font-sans pb-20">
      <div className="w-full max-w-[1000px] mx-auto px-6 pt-10">
        <Link href="/careerdna/admin/profiles" className="text-[#fb6a51] font-bold flex items-center gap-2 mb-8 hover:translate-x-[-4px] transition-transform w-fit">
          ← Back to Profiles
        </Link>
