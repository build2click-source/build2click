...
export default function CareerDNADashboard() {
  ...
  useEffect(() => {
    ...
    if (sessionStatus === "authenticated") {
      fetch("/api/careerdna/user-assessments")
        .then((r) => r.json())
        .then((data) => {
                ...
                <div className={`flex flex-col w-full sm:flex-row items-center justify-center gap-4 shrink-0 ${asmt.status !== 'NOT_STARTED' ? 'border-t border-slate-100 pt-8 mt-2' : 'mt-4'}`}>
                  {asmt.status === "COMPLETED" && asmt.attemptId ? (
                    <div className="flex flex-col w-full sm:w-auto gap-3">
                      <Link href={`/careerdna/results/${asmt.attemptId}`} className="w-full">
                      ...
                      {(asmt.attemptsRemaining > 0 || (session?.user as any)?.role === "ADMIN") && (
                        <Link href={`/careerdna/dashboard/${asmt.id}`} className="w-full">
                    ...
                    <Link href={`/careerdna/dashboard/${asmt.id}`} className="w-full sm:w-auto">
                      <button 
                          ...
                          <div className="mt-3 sm:mt-0 ml-12 sm:ml-0 flex-[0.8] sm:flex-none">
                            {past.isCompleted ? (
                               <div className="flex items-center justify-between sm:justify-end gap-4 w-full">
                                  ...
                                  <Link href={`/careerdna/results/${past.id}`} className="flex items-center justify-center shrink-0 size-8 sm:size-10 bg-slate-50 hover:bg-[#fb6a51]/10 text-slate-400 hover:text-[#fb6a51] rounded-full transition-all border border-slate-100">
