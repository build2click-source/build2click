...
export default function CareerDNAAssessmentModuleMapPage() {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && assessmentId) {
      Promise.all([
        fetch(`/api/careerdna/assessment/${assessmentId}`).then((r) => r.json()),
        fetch(`/api/careerdna/attempt/module-progress?assessmentId=${assessmentId}`).then((r) => r.json()),
      ])
        .then(([asmtData, progressData]) => {
          ...
          if (asmtData.modules && !isAdmin) {
            ...
            if (totalQ > 0 && answeredQ >= totalQ && progressData.attemptId) {
              router.push(`/careerdna/results/${progressData.attemptId}`);
              return;
            }
          }
          setLoading(false);
  ...
  }, [status, router, assessmentId, isAdmin]);
  ...
  if (!assessment) {
    return (
      <div className="min-h-screen bg-[#F4F7F6] flex items-center justify-center flex-col gap-4">
        ...
        <Link href="/careerdna/dashboard" className="text-[#fb6a51] font-bold">← Back to Dashboard</Link>
      </div>
    );
  }

  const totalAnswered = Object.values(progress).reduce((s, p) => s + p.answered, 0);
  const totalQuestionsGlobal = assessment.modules.reduce((s, m) => s + m._count.questions, 0);
  ...
  return (
    <div className="text-[#2D3142] min-h-screen flex flex-col items-center bg-[#F4F7F6] font-sans pb-20">
      ...
      <main className="w-full max-w-[800px] px-6 pt-10 flex flex-col gap-10">
        ...
        <Link href="/careerdna/dashboard" className="text-[#fb6a51] font-bold flex items-center gap-2 hover:translate-x-[-4px] transition-transform w-fit -mb-4">
          ← Back to Assessments
        </Link>
                ...
                <button
                  onClick={async () => {
                    if (confirm("Reset everything? Your current answers will be finalized and a fresh attempt started.")) {
                      setLoading(true);
                      await fetch("/api/careerdna/admin/reset-attempt", {
                        method: "POST",
            ...
            return assessment.modules.map((mod, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <Link href={isComplete && !isAdmin ? "#" : `/careerdna/assessment/${mod.id}?assessmentId=${assessment.id}`} className={(!isUnlocked && !isAdmin || (isComplete && !isAdmin)) ? "cursor-default pointer-events-none w-full md:w-auto" : "w-full md:w-auto"}>
                    <button
