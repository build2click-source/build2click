...
export default function CareerDNAAssessmentEnginePage() {
  ...
  useEffect(() => {
    ...
    if (status !== "authenticated" || !moduleId) return;

    setLoading(true);

    // 1. Fetch module questions
    // In CareerDNA, we can fetch by moduleId or type. The original used moduleId in path but mapping was a bit loose.
    // Let's assume the API we built takes the ID.
    fetch(`/api/careerdna/assessment/module/${moduleId}/questions`)
      .then((r) => r.json())
      .then(async (data) => {
        ...
        if (data.assessmentId) {
          const attemptRes = await fetch("/api/careerdna/attempt/start", {
            method: "POST",
          ...
          if (attemptData.attemptId) {
            ...
            const progressRes = await fetch(
              `/api/careerdna/attempt/progress?attemptId=${attemptData.attemptId}&moduleId=${data.moduleId}`
            );
            ...
            if (progressData.answeredMap) {
              ...
              try {
                const lsRaw = localStorage.getItem(`careerdna_buffer_${attemptData.attemptId}`);
              ...
              let resumeIdx = questions.length;
  ...
  const syncAnswers = useCallback(async (currentAnswersObj: Record<string, number>) => {
    ...
    try {
      await fetch("/api/careerdna/attempt/respond", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ responses: responsesPayload }),
      });
      localStorage.removeItem(`careerdna_buffer_${attemptId}`);
    } catch (err) {
  ...
  const handleSelect = useCallback((score: number) => {
    ...
    if (attemptId) {
       try {
         const existing = JSON.parse(localStorage.getItem(`careerdna_buffer_${attemptId}`) || "{}");
         existing[currentQuestion.id] = score;
         localStorage.setItem(`careerdna_buffer_${attemptId}`, JSON.stringify(existing));
       } catch(e) {}
    }
    ...
    setTimeout(async () => {
      if (currentIdx < totalQuestions - 1) {
        ...
        if (moduleData.isLastModule && attemptId) {
          router.push(`/careerdna/results/${attemptId}`);
        } else {
          router.push(`/careerdna/dashboard/${moduleData.assessmentId}`);
        }
      }
    }, 350);
  ...
  if (error || !moduleData) {
    return (
      <div className="min-h-screen bg-[#f8f6f5] flex flex-col items-center justify-center gap-4">
        ...
        <Link href="/careerdna/dashboard" className="text-[#fb6a51] underline">← Back to Dashboard</Link>
      </div>
    );
  }
          ...
          <header className="flex flex-col md:flex-row items-center justify-between px-6 md:px-10 py-4 md:h-[72px] gap-4 md:gap-0">
            ...
            <div className="flex-1 w-full max-w-md md:mx-8 flex flex-col gap-1.5 order-last md:order-none">
              <div className="flex justify-between items-end px-1">
                <p className="text-[10px] font-bold text-[#2D3142] uppercase tracking-wider">{moduleData.title}</p>
                <p className="text-sm font-bold text-[#2D3142]">{currentIdx + 1} / {totalQuestions}</p>
              </div>
          ...
          {!isMultipleChoice && (
            <div className="w-full flex flex-col items-center gap-4">
              <div className="flex justify-between w-full max-w-[380px]">
                {emojis.map((emoji, i) => {
                  const score = i + 1;
                  const isSelected = selectedScore === score || (currentQuestion && answers[currentQuestion.id] === score);
                  return (
                    <button
                      key={i}
          ...
          {isMultipleChoice && (
            <div className="w-full flex flex-col gap-3">
              {currentQuestion?.options.map((opt, i) => {
                const isSelected = selectedScore === i + 1 || (currentQuestion && answers[currentQuestion.id] === i + 1);
                return (
                  <button
                    key={i}
