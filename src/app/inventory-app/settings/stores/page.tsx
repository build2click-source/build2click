...
export default function StoresPage() {
    ...
    const fetchStores = async () => {
        setLoading(true);
        const res = await fetch('/inventory-app/api/stores');
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingStore) {
                await fetch(`/inventory-app/api/stores/${editingStore.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
            } else {
                await fetch('/inventory-app/api/stores', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
            }
            closeForm();
            fetchStores();
        } catch {
    ...
    const handleDelete = async (id: string, name: string) => {
        ...
        await fetch(`/inventory-app/api/stores/${id}`, { method: 'DELETE' });
        fetchStores();
    };

    const toggleActive = async (s: StoreData) => {
        await fetch(`/inventory-app/api/stores/${s.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isActive: !s.isActive }) });
        fetchStores();
    };
