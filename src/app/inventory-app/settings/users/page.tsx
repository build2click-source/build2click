...
export default function UsersPage() {
    ...
    const fetchUsers = async () => {
        setLoading(true);
        const res = await fetch('/inventory-app/api/users');
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (editingUser) {
                await fetch(`/inventory-app/api/users/${editingUser.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
            } else {
                const res = await fetch('/inventory-app/api/users', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    ...
    const handleDelete = async (u: User) => {
        ...
        await fetch(`/inventory-app/api/users/${u.id}`, { method: 'DELETE' });
        fetchUsers();
    };
