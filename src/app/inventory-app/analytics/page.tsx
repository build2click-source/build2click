...
export default function AnalyticsDashboard() {
    ...
    useEffect(() => {
        setIsLoading(true);
        const url = storeId ? `/inventory-app/api/analytics?period=${period}&storeId=${storeId}` : `/inventory-app/api/analytics?period=${period}`;
    ...
    useEffect(() => {
        ...
        if (stores.length > 1) {
            Promise.all(stores.map(s => fetch(`/inventory-app/api/analytics?period=${period}&storeId=${s.id}`).then(r => r.json()).then(d => ({ store: s.branch || s.name, revenue: d.revenue?.totalEarnings || 0, paid: d.revenue?.totalPaid || 0 })))).then(setStoreData);
        }
    }, [period, stores]);
