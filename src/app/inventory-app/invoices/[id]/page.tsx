...
export default function InvoiceViewer() {
    ...
    const savePayment = async (status: string, amount?: number) => {
        ...
        const res = await fetch(`/inventory-app/api/invoices/${invoice.id}`, {
            method: 'PATCH',
    ...
    useEffect(() => {
        setIsClient(true);
        fetch('/inventory-app/api/settings').then(r => r.json()).then(d => setGstEnabled(d.gst_enabled !== 'false'));
        fetch(`/inventory-app/api/invoices/${id}`)
            .then(r => r.json())
    ...
    if (error || !invoice) return (
        <div style={{ padding: '4rem', textAlign: 'center' }}>
            <div className="card glass-panel" style={{ display: 'inline-block', padding: '3rem' }}>
                ...
                <Link href="/inventory-app/invoices" className="btn-secondary" style={{ marginTop: '1.5rem', display: 'inline-flex' }}>
    ...
    return (
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 350px', gap: '2rem' }}>
            ...
            <div className="action-sidebar" style={{ position: 'sticky', top: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                <Link href="/inventory-app/invoices" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#64748b', textDecoration: 'none', fontSize: '0.875rem', fontWeight: 600 }}>
