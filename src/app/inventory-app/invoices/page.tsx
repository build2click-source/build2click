...
export default function InvoicesPage() {
    ...
    useEffect(() => {
        if (searchQuery.length > 1) {
            fetch(`/inventory-app/api/invoices?q=${encodeURIComponent(searchQuery)}`).then(r => r.json()).then(setSuggestions);
        } else setSuggestions([]);
    }, [searchQuery]);
                                    ...
                                    return (
                                        <tr key={inv.id} style={{ borderBottom: '1px solid #f1f5f9', borderLeft: `4px solid ${status.border}`, transition: 'background 0.2s' }}
                                            ...
                                            <td style={{ padding: '1rem' }}>
                                                <Link href={`/inventory-app/invoices/${inv.id}`} style={{ fontWeight: 700, color: 'var(--primary)', textDecoration: 'none' }}>
                                                    {inv.invoiceNumber}
                                                </Link>
                                            ...
                                            <td style={{ padding: '1rem', textAlign: 'center' }}>
                                                <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', flexWrap: 'wrap' }}>
                                                    <Link href={`/inventory-app/invoices/${inv.id}`}
