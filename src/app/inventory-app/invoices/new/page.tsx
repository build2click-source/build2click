...
export default function MakeInvoicePage() {
    ...
    useEffect(() => {
        ...
        fetch('/inventory-app/api/stores').then(r => r.json()).then((d: StoreOption[]) => {
            setStores(d);
    ...
    useEffect(() => {
        if (searchQuery.length > 1) {
            const q = storeId ? `/inventory-app/api/products?q=${encodeURIComponent(searchQuery)}&storeId=${storeId}` : `/inventory-app/api/products?q=${encodeURIComponent(searchQuery)}`;
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await fetch('/inventory-app/api/invoices', {
                method: 'POST',
            ...
            <div className="container" style={{ maxWidth: '600px', textAlign: 'center', paddingTop: '4rem' }}>
                <div className="card glass-panel" style={{ padding: '3rem 2rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                        ...
                        <button className="btn-primary" onClick={() => router.push(`/inventory-app/invoices/${successId}`)}>
